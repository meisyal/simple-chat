/* to compile me in Linux, type:  gcc -o server t1.c -lpthread */  
  ...
  *     the server uses the default given by PORT.*/  
   ...
   typedef ptrtonode addr;  
      ...
      void DisplayClient( const head h, int socketfd); 
 ...
 int main(int argc, char *argv[]) {  
  ...
  if( argc == 2 )       
   ...
   while(1){  
	     ...
	     char *pch=strtok(username,":");
	     pch=strtok(NULL,":");  
	     username[0]='\0';
	     strcpy(username,pch);
             printf("\t%d->%s JOINED chatroom\n",new_fd,username);  
             sprintf(buffer,"Done: %s\n",username);  
 ...
 /*========displaying online list in client==================*/  
 void DisplayClient( const head h, int socketfd)  
      {  
        addr a =h ;  
        if( h->next == NULL )  
